...
selectedSharedFishmarks: [...state.selectedSharedFishmarks.filter(mark =>
mark.latitude !== action.position.latitude
&& mark.longitude !== action.position.longitude)],
loadedWaypoints: [...state.loadedWaypoints.filter(mark =>
mark.latitude !== action.position.latitude
&& mark.longitude !== action.position.longitude)],
myFishmarkWaypoints: [...state.myFishmarkWaypoints.filter(mark =>
   mark.latitude !== action.position.latitude
  && mark.longitude !== action.position.longitude)],
};
...
return {
...state, fishmarks: [], message: action.message, isFetching: action.isFetching,
};
...
return {
  ...state,
  fishmarks:
  action.toSaveWaypoints,
  sharedFishmarks:
  action.filteredSharedWaypoints,
};
...
return {
  ...state,
  sharedFishmarks:
  action.sharedFishmarks,
  sharedFishmarksNumber:
  action.sharedFishmarksNumber,
};
case SHARE_WAYPOINT_CHECKED_SUCCESS:
return {
  ...state,
  sharedFishmarksNumber: action.number,
...
return {
  ...
  sharedFishmarks: [...state.sharedFishmarks.map(fishmark => ({
      ...fishmark,
      checked: fishmark.key === action.selectedSharedFishmark.key ?
      action.selectedSharedFishmark.checked :
      fishmark.checked,
    }))],
};
...
return {
  ...state,
  myFishmarkWaypoints: [...state.myFishmarkWaypoints.filter(mark =>
  mark.latitude !== action.data.latitude
